...
let firstNamesAllCaps = runners.map(r => r.first_name.toUpperCase())
...
let ticketPriceTotal = runners.reduce((t, item) => t + parseFloat(item.donation), 0);
...
// Problem 1
// All small sized shirts donation 
var smallShirtsDonation = runners.filter(r => r.shirt_size == "S").reduce((t, i) => t + parseFloat(i.donation), 0)
console.log(smallShirtsDonation)
// Problem 2
// All the emails that have donation greater than 200
var mailsDonation = runners.filter(r => parseFloat(r.donation) > 200).map(r => r.email);
console.log(mailsDonation)
// Problem 3
// find avg donation for each person with size as M
var avg_donation = runners.filter(r => r.shirt_size == "M").reduce((t, i) => t + parseFloat(i.donation), 0) / runners.filter(r => r.shirt_size == "M").length
console.log(avg_donation);